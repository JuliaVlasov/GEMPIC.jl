...
function spline_fem_mass_line(degree::Int)
    ...
    for j in 1:(degree + 1), i in j:(degree + 1), k in 1:n
...
function compute_rhs_from_function(solver::TwoDPoisson, f)
    ...
    for i2 in 1:ny, i1 in 1:nx
        ...
        for j1 in 1:(d + 1), j2 in 1:(d + 1)
            # loop over Gauss points
            for k1 in 1:(d + 1), k2 in 1:(d + 1)
                x = dx * (xg[k1] + (i1 + j1 - 2))
                y = dy * (xg[k2] + (i2 + j2 - 2))
                coef +=
                    wg[k1] *
...
function compute_e_from_rho!(efield, solver::TwoDPoisson, rho)
    ...
    for j in 1:ny, i in 1:nx
        if (i == 1 && j == 1)
            ...
            eig_val = (
                solver.eig_values_dtm1d_1[i] * solver.eig_values_mass_0_2[j] +
                solver.eig_values_mass_0_1[i] * solver.eig_values_dtm1d_2[j]
            )
    ...
    return ifft2d!(efield[2], solver, solver.scratchy)
end
